...
login_if_needed() {
  ...
  if test -n "${log_file_path}" && wait_for_message "${timeout}" 'peer has been successfully registered|management connection state READY'; then
    ...
    info "logging in..."
    if test -n "${NB_SETUP_KEY:-}"; then
      info "using setup key for authentication..."
      "${NETBIRD_BIN}" up --setup-key "${NB_SETUP_KEY}"
    else
      info "no setup key provided, using default SSO login..."
      "${NETBIRD_BIN}" up
    fi
  fi
}
...
main() {
  ...
  ##########################################
  info "starting redirect_pkg_handler..."
  /redirect_pkg_handler -m route &
  service_pids+=("$!")
  info "registered redirect_pkg_handler, running: ${service_pids[@]@Q}"
  ##########################################
